...
public final class FastMineMiningRuleConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        verticalButton = addRenderableWidget(Button.builder(Component.empty(), button -> save(true, false, false, false)).bounds(left, 108, 145, 20).build());
        protectionButton = addRenderableWidget(Button.builder(Component.empty(), button -> save(false, true, false, false)).bounds(left + 154, 108, 145, 20).build());
        veinSneakButton = addRenderableWidget(Button.builder(Component.empty(), button -> save(false, false, true, false)).bounds(left, 136, 145, 20).build());
        areaSneakButton = addRenderableWidget(Button.builder(Component.empty(), button -> save(false, false, false, true)).bounds(left + 154, 136, 145, 20).build());
        saveButton = addRenderableWidget(Button.builder(Component.literal("保存数值规则"), button -> save(false, false, false, false)).bounds(left, 164, 299, 20).build());
    ...
    private void refreshWidgets() {
        ...
        verticalButton.active = available; protectionButton.active = available; veinSneakButton.active = available; areaSneakButton.active = available;
        if (!available) return;
